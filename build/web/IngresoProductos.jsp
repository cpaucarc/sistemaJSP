...
<html>
    ...
    <body>
          ...
          $('#rucprov').on('input', function() {
            if($('#rucprov').val().length===11){
             ...
             $.ajax({
              ...
              success: function (msg) {$('#elprv').text(msg.trim());
               if(msg.trim().length>0){                
                OculAparecer(1);document.getElementById('cbcomp').selectedIndex=-1;     
               }                   
              },
              error: function (xml,msg) {}                    
             });             
            }
            else{OculAparecer(0);$('#elprv').text(""); }                        
           });          
      ...
      function Mostrar(id,dt){
       ...
       $.ajax({
        ...
        success: function (msg) {
         $("#data1").html(msg);
         //$("#data2").html(msg);
         ordtabla('tabla1');$(".dataTables_filter").hide();
        },
      ...
      function OculAparecer(bdr){
       if(bdr===1){$('#tpocbt').show();$('#cbcomp').show();$("#num").show();}   
       else{$('#tpocbt').hide();$('#cbcomp').hide();$("#num").hide();}           
      } 
